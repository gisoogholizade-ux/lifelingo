...
let sb=null,data=null,gender=null,claiming=false,refreshing=false;
const META={airport:{paid:false},sim:{paid:false,prev:['migration',0]},transport:{paid:true,prev:['migration',1]},shopping:{paid:true,prev:['migration',2]},bank:{paid:true,prev:['migration',3]},doctor:{paid:true,prev:['migration',4]},apartment:{paid:true,unit:'c3-conversation'},workplace:{paid:true},interview:{paid:true,prev:['career',0]},hotel:{paid:true},restaurant:{paid:true,prev:['travel',0]},directions:{paid:true,prev:['travel',1]}};
...
async function refresh(){if(refreshing)return;refreshing=true;try{sb=window.llSupabase||sb;if(!sb)return;const s=await sb.auth.getSession();if(!s.data?.session){data=null;return}const r=await sb.rpc('get_unified_profile');if(!r.error)data=r.data}catch(e){console.warn('LifeLingo entitlement refresh',e)}finally{refreshing=false;patch()}}
...
window.addEventListener('lifelingo:profile-updated',refresh);window.addEventListener('lifelingo:mission-saved',refresh);window.addEventListener('hashchange',()=>setTimeout(refresh,40));
