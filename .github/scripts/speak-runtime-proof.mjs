...
await page.addInitScript(() => {
  const D = window.__SPEAK_PROOF = {regs:[],fires:[],hashchanges:0,popstates:0,pushStates:0,replaceStates:0,mutationCallbacks:0,mutationRecords:0};
  ...
  const NativeMO=window.MutationObserver;window.MutationObserver=class extends NativeMO{constructor(cb){super((records,obs)=>{D.mutationCallbacks++;D.mutationRecords+=records.length;return cb(records,obs)})}};
});
...
const home=page.locator('[data-nav="home"]:visible, [data-header-nav="home"]:visible').first();if(await home.count()){try{await home.click({timeout:3000});await page.waitForTimeout(100)}catch{}}
...
const timer=await page.evaluate(()=>new Promise(resolve=>{const a=performance.now();setTimeout(()=>resolve(performance.now()-a),100)}));console.log('MAIN_THREAD_100MS_TIMER',timer);
console.log('RUNTIME_COUNTERS_AND_HANDLERS',JSON.stringify(await page.evaluate(({a,b})=>({...window.__SPEAK_PROOF,firesForSpeakClick:window.__SPEAK_PROOF.fires.slice(a,b)}),{a:fireStart,b:fireEnd})));
for(const[name,selector]of[['Home','[data-nav="home"]:visible, [data-header-nav="home"]:visible, [data-fixed-nav="home"]:visible'],['Learn','[data-nav="learn"]:visible, [data-header-nav="learn"]:visible, [data-fixed-nav="learn"]:visible'],['Partners','[data-nav="partners"]:visible, [data-header-nav="partners"]:visible, [data-fixed-nav="partners"]:visible'],['Profile','[data-nav="profile"]:visible, [data-header-nav="profile"]:visible, [data-fixed-nav="profile"]:visible, #topAvatar:visible']]){try{const el=page.locator(selector).first();await el.click({timeout:2500});await page.waitForTimeout(100);console.log(`POST_SPEAK_${name}_CLICK_OK`,JSON.stringify({url:page.url(),active:await page.evaluate(()=>[...document.querySelectorAll('.view.on')].map(x=>x.id))}))}catch(e){console.log(`POST_SPEAK_${name}_CLICK_FAIL`,e.message)}}
await browser.close();
