...
class MockAIProvider implements AIProvider{
  ...
  async generate(_system:string,userPayload:string){const input=JSON.parse(userPayload),message=String(input.learnerInput||''),lower=message.toLowerCase(),scenario=input.scenarioId||'',turn=Number(input.turnNumber||0),completed:string[]=[];const mistakes:TurnResult['mistakes']=[];
    ...
    if(scenario==='airport'){
      ...
      if(/developer|student|engineer|experience|work/.test(lower))completed.push('introduce_self');if(/project|built|developed|created/.test(lower))completed.push('describe_project');if(/role|responsible|backend|frontend|api/.test(lower))completed.push('explain_role');if(/grow|contribute|motivated|motivation|interested in|join (the|your) team|join (the|your) company/.test(lower))completed.push('explain_motivation');
