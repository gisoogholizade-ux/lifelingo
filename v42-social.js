...
const faceStyle=i=>{const a=typeof i==='object'?i:byId(i);return `background-image:url('${a.src}?v=confession2');background-size:cover;background-position:center;background-repeat:no-repeat;background-color:#0b1120`};
...
function reveal(g,h){const s=load();if(s.surpriseUsed)return panel(h);const pool=avatars.filter(a=>a.cat===g),a=pool[Math.floor(Math.random()*pool.length)];s.avatar=a.id;s.surpriseUsed=true;s.surpriseGender=g;save(s);panel(h);syncProfileButton()}
