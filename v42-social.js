...
const state=()=>{const u=user();const a=bySrc(u?.avatarUrl||'');return{avatar:a?.id??0,surpriseUsed:!!a,surpriseGender:a?.cat||null}};
const displayName=()=>{const u=user()||{};return u.name||u.displayName||u.fullName||(u.email?String(u.email).split('@')[0]:'You')};
const faceStyle=i=>{const a=typeof i==='object'?i:byId(i);return `background-image:url('${a.src}?v=avatar4');background-size:cover;background-position:center;background-repeat:no-repeat;background-color:#0b1120`};
const face=(i,cls='')=>{const a=typeof i==='object'?i:byId(i);return `<img class="ll43Face ${cls}" src="${a.src}?v=avatar4" alt="LifeLingo avatar" loading="eager" decoding="async">`};
...
async function saveAvatar(a){const u=user();if(!u?.id||!window.llSupabase)return false;const {error}=await window.llSupabase.from('profiles').update({avatar_url:a.src}).eq('id',u.id);if(error)throw error;const next={...u,avatarUrl:a.src};localStorage.setItem('lifelingo_user',JSON.stringify(next));try{user=next}catch{}return true}
async function reveal(g,h){const current=state();if(current.surpriseUsed)return panel(h);const pool=avatars.filter(a=>a.cat===g);if(!pool.length)return;const a=pool[randomIndex(pool.length)];const buttons=[...h.querySelectorAll('button')];buttons.forEach(b=>b.disabled=true);try{await saveAvatar(a);panel(h);syncProfileButton()}catch(err){h.querySelector('[data-panel]').innerHTML=`<div class="llSurpriseIntro"><div class="llSurpriseOrb">!</div><h3>Could not save your avatar</h3><p>${String(err?.message||'Please try again.')}</p><button class="ll42Share" data-surprise>Try again</button></div>`}finally{buttons.forEach(b=>b.disabled=false)}}
...
window.addEventListener('lifelingo:user-changed',()=>{document.querySelector('#ll42Social')?.remove();resetProfileButton();setTimeout(()=>{render();syncProfileButton()},50)});
setTimeout(()=>{render();syncProfileButton()},700);setInterval(()=>{if(user()&&!$('#ll42Social'))render();syncProfileButton()},1200);
})();