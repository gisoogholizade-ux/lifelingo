...
const key=()=>{const u=user();const identity=u?.id||u?.email;if(!identity)return 'lifelingo_social_signed_out';return `lifelingo_social_${String(identity).toLowerCase()}`};
const avatars=[
...
const faceStyle=i=>{const a=typeof i==='object'?i:byId(i);return `background-image:url('${a.src}?v=avatar3');background-size:cover;background-position:center;background-repeat:no-repeat;background-color:#0b1120`};
const face=(i,cls='')=>{const a=typeof i==='object'?i:byId(i);return `<img class="ll43Face ${cls}" src="${a.src}?v=avatar3" alt="LifeLingo avatar" loading="eager" decoding="async">`};
...
window.addEventListener('lifelingo:user-changed',()=>{document.querySelector('#ll42Social')?.remove();setTimeout(()=>{render();syncProfileButton()},0)});
